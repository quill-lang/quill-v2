...
\begin{mathparpagebreakable}
  ...
  \inferrule*[right=$\Sigma$-\relim]
    {\oftp{\Gamma}{B}{A \to \UU} \\
     ...
     \oftp{\Gamma}{g}{\tprd{x:A} \tprd{y:B} C(\dtup x y)} \\
...
% TODO: add a way to call (A x) where A is a named version of a type T->U, and x:T, which will unbox the named type, call the function, then box in a new name
...
\begin{tabular}{r l p{7cm}}
    ...
    \lstinline!defn! \( ::= \) & \lstinline!(def DefInfo* Name Expr)! \\
