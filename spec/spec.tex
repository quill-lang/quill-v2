...
% TODO: THIS IS WHERE THE RIGOUR STARTS; this should be made clear by use of section delimiters
...
Feather has several primitive types that have not yet been discussed, such as the unit type \( \unit \) and the 64-bit signed integer type \( \mathbb Z_{64} \).
In the following section, let \( \mathbb P \) denote a primitive type and let \( p \) be an arbitrary element of type \( \mathbb P \), as defined in \cref{sec:prim_types}.
\begin{mathparpagebreakable}
...
\\
  ...
  {\oftp{\Gamma}{C}{\bigplus_{j=1}^n A_j \to \UU} \\\\
   \oftp{\Gamma}{c_1}{\tprd{x_1:A_1} C(\inj_1(x))} \\
   \cdots \\
   \oftp{\Gamma}{c_n}{\tprd{x_n:A_n} C(\inj_n(y))} \\\\
...
\begin{tabular}{r l p{7cm}}
    ...
    \inlinecode{ExprContents} \( ::= \) & \makecell[l]{
        ...
        % TODO: add support for all the section 2 type rules
        \inlinecode{| universe}
    } \\
