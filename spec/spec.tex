...
\begin{defn}
  ...
  This symbolises a lossy conversion of \( x \) into \( y \) that cannot necessarily be trivially reversed.
\end{defn}

\subsection{Ownership}
\begin{defn}
  A one-directional computation of \( a \) into \( b \) \textit{respects ownership} if all free variables that appear in \( x \) appear no more than once in \( y \).
  A computation of \( a \) into \( b \) \textit{respects ownership} if both one-directional computations respect ownership.
  Equivalently, it respects ownership if all free variables that appear in \( x \) appear exactly once in \( y \), and vice versa.
\end{defn}
\begin{eg}
...
% TODO: this is a pretty bad description of computation rules, also we need to change \equiv into \mapsto in all the rules below
...
\begin{mathparpagebreakable}
  ...
  {\oftp{\Gamma}{A}{\UU} \and \oftp{\Gamma}{B}{\UU} \and \oftp{\Gamma,\tmtp xA}{x.b}{B} \and \oftp\Gamma{a}{A}}
...
\and
  ...
  {\oftp{\Gamma}{A}{\UU} \and \oftp{\Gamma}{B}{\UU} \and \oftp{\Gamma,\tmtp xA}{x.b}{B}}
...
\and
  ...
  {\oftp{\Gamma}{B}{\UU} \\ \oftp{\Gamma,\tmtp xA}{x.b}{B} \\ \oftp\Gamma{a}{A}}
...
\begin{mathparpagebreakable}
  ...
  {\oftp{\Gamma}{C}{\mathbb B \to \UU} \\\\
   \oftp{\Gamma}{c_\bot}{\tprd{x:\unit} C(\bot)} \\
   \oftp{\Gamma}{c_\top}{\tprd{x:\unit} C(\top)} \\\\
...
\\
  ...
  {\oftp\Gamma{A_i}{\UU} \\ \oftp\Gamma{a}{A_i}}
...
\and
  ...
  {\oftp{\Gamma}{C}{\bigplus_{j=1}^n A_j \to \UU} \\
   ...
   \oftp{\Gamma}{c_n}{\tprd{x_n:A_n} C(\inj_n(x_n))} \\\\
   \oftp\Gamma{a}{A_j}}
  {\jdeqtp\Gamma{\ind{\bigplus_{j=1}^n A_j}(C,c_1,\dots,c_n,\inj_j(a))}{c_j(a)}{C(\inj_j(a))}}
\end{mathparpagebreakable}
...
This is known as the \lstinline{newtype} idiom.
Let \( \mathcal I \) be the type of \textit{unique identifiers}, defined by the following rules.

\begin{mathparpagebreakable}
  \inferrule*[right=$\mathcal I$-\textsc{qn}]
    {\oftp{\Gamma}{s}{\mathtt{QualifiedName}}}
    {\oftp\Gamma{I_{\textsc{qn}}(s)}{\mathcal I}}
  \and
  \inferrule*[right=$\mathcal I$-\textsc{app}]
    {\oftp{\Gamma}{i}{\mathcal I} \and \oftp{\Gamma}{A}{\UU} \and \oftp{\Gamma}{a}{A}}
    {\oftp\Gamma{I_{\textsc{app}}(i,a)}{\mathcal I}}
\end{mathparpagebreakable}

Informally, we can construct an identifier out of a name, or the application of a named type to an arbitrary value.
We will allow values (typically types or functions which return types) to be given an identifier by defining the following rules.

\begin{mathparpagebreakable}
  \inferrule*[right=$\mathcal N$-\textsc{qn}-\rform]
    {\oftp{\Gamma}{A}{\mathcal A} \and \oftp{\Gamma}{s}{\mathtt{QualifiedName}}}
    {\oftp\Gamma{\mathcal N(I_{\textsc{qn}}(s), A)}{\UU}}
  \and
  \inferrule*[right=$\mathcal N$-\textsc{app}-\rform]
    {\oftp\Gamma{A}{B \to \UU} \\
     \oftp\Gamma{\mathcal N(i, A)}{\UU} \\
     \oftp\Gamma{b}{B}}
    {\oftp\Gamma{\mathcal N(I_{\textsc{app}}(i,b), A(b))}{\UU}}
  \and
  \inferrule*[right=$\mathcal N$-\textsc{qn}-\rintro]
    {\oftp{\Gamma}{A}{\mathcal A} \\
    \oftp{\Gamma}{s}{\mathtt{QualifiedName}} \\ \oftp\Gamma{a}{A}}
    {\oftp\Gamma{\nu(I_{\textsc{qn}}(s), a)}{\mathcal N(I_{\textsc{qn}}(s), A)}}
  \and
  \inferrule*[right=$\mathcal N$-\textsc{app}-\rintro]
    {\oftp\Gamma{A}{B \to \UU} \\
    \oftp\Gamma{\mathcal N(i, A)}{\UU} \\
    \oftp\Gamma{b}{B} \\
    \oftp\Gamma{c}{A(b)}}
    {\oftp\Gamma{\nu(I_{\textsc{app}}(i,b), c)}{\mathcal N(I_{\textsc{app}}(i,b), A(b))}}
  \and
  \inferrule*[right=$\mathcal N$-\relim]
    {\oftp{\Gamma}{A}{\UU} \\
     \oftp{\Gamma}{i}{\mathcal I} \\
     \oftp{\Gamma}{C}{\mathcal N(i, A) \to \UU} \\\\
     \oftp{\Gamma}{g}{\tprd{x:A} C(\nu(i, x))} \\
     \oftp\Gamma{p}{\mathcal N(i, A)}}
    {\oftp\Gamma{\ind{\mathcal N(i, A)}(C,g,p)}{C(p)}}
  \and
  \inferrule*[right=$\mathcal N$-\rcomp]
    {\oftp{\Gamma}{A}{\UU} \\
     ...
     \oftp{\Gamma}{g}{\tprd{x:A} C(\nu(i,x))} \\
     \oftp\Gamma{a}{A}}
    {\jdeqtp\Gamma{\ind{\mathcal N(i,A)}(C,g,\nu(i,a))}{g(a)}{C(\nu(i,a))}}
\end{mathparpagebreakable}

Note that a named type is `boxed'; in order to address the underlying type you must `unbox' the newtype wrapper first.
This unboxing operation will be assigned a visibility later in compilation, allowing only certain modules to unbox a given type.
Note also that the induction operation is independent of the name given to a type; the name simply acts as a marker for the compiler for more semantic knowledge about a type.
...
\end{mathparpagebreakable}
We say that the $\mathcal B$-\rintro{} rule does \textit{not} consume the variable \( a \).
...
\begin{proof}
  ...
  \end{align*}
  From now, we may write \( \ind{\bigtimes_{j=1}^n A_j} \) to refer to this new \( \ind{\bigtimes_{j=1}^n A_j}' \).
  Whether we are using the induction principle as a function or as an atomic syntactic element should be made clear from context.
\end{proof}
...
The induction function does not necessarily respect ownership in the same way that the atomic induction principle does.
...
For the purposes of eliding de Bruijn indices, we will assume that the Feather expression for creating an abstraction, \lstinline{(lambda e)}, is instead written \( \lam xe \), where \( x \) is bound in the expression \( x \).
...
% TODO: maybe add structural rules from hott
\[
  \inferrule*[right=\(\to\)-\rintro]{
    \inferrule*[right=\(\to\)-\rform]{
...
\begin{lstlisting}[mathescape=true]
(app $e:A \to B$ Expr $x:A$ DeBruijnIndex)$ \;\longleftrightarrow\; e\ x:B $
\end{lstlisting}
...
\begin{lstlisting}[mathescape=true]
(if $f: \unit \to C$ Expr $t: \unit \to C$ Expr $b: \mathbb B$ Expr)$ \;\longleftrightarrow\; \ind{\mathbb B}(C,f,t,b): C $
\end{lstlisting}
...
Types that differ only in their component information are considered to be equal; the component information is used primarily inside the context of a newtype, where the difference is made semantic.
...
The prefix \lstinline{r} refers to the recursor.
\begin{lstlisting}[mathescape=true]
...
\end{lstlisting}
Note that the recursor is used atomically here, not as a function.
We similarly define
\begin{lstlisting}[mathescape=true]
...
\subsection{Newtypes}
Newtypes have their own information type.

\begin{tabular}{r l p{7cm}}
  \lstinline!NewtypeInfo! \( ::= \) & \lstinline!(name !\( \mathbb S \)\lstinline!) | (doc !\( \mathbb S \)\lstinline!)! \\
\end{tabular}

We can construct newtypes using the following expressions.

\begin{lstlisting}[mathescape=true]
(fnewtyqn NewtypeInfo* $A:\mathcal A$ Expr $s:\ $QualifiedName)$ \;\longleftrightarrow\; \mathcal N(I_{\textnormal{\textsc{qn}}}(s), A): \UU $
(fnewtyapp NewtypeInfo* $A':\mathcal N(i, A: B \to C)$ Expr $b:B$ Expr)
    $\longleftrightarrow\; \mathcal N(I_{\textnormal{\textsc{app}}}(i,b), A(b)): \UU $
(inewtyqn $a:A$ Expr)$\;\longleftrightarrow\; \nu(I_{\textnormal{\textsc{qn}}}(s), a):\mathcal N(I_{\textnormal{\textsc{qn}}}(s), A) $
(inewtyapp $c:A(b)$ Expr)$\;\longleftrightarrow\; \nu(I_{\textnormal{\textsc{app}}}(i,b), c):\mathcal N(I_{\textnormal{\textsc{app}}}(i,b), A(b)) $
(rnewty $g : A \to C$ Expr $p:\mathcal N(i, A)$ Expr)$\;\longleftrightarrow\; \rec{\mathcal N(i, A)}(C,g,p): C $
\end{lstlisting}

In the \lstinline{fnewtyapp} expression, the syntax \( A':\mathcal N(i, A: B \to C) \) means both \( A':\mathcal N(i, A) \) with the restriction that \( A: B \to C \), but has been shortened for ease of writing.
Note that the type of the \lstinline{inewty*} expressions can be used to deduce the named types in question.
