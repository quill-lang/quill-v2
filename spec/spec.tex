...
To aid in clarifying semantic meaning, we permit fields in a product type to be named.
...
We will allow types to be given an identifier by defining the following rules.

\begin{mathparpagebreakable}
  \inferrule*[right=$\mathcal N$-\textsc{qn}-\rform]
    {\oftp{\Gamma}{A}{\UU} \and \oftp{\Gamma}{s}{\mathtt{QualifiedName}}}
  ...
  \inferrule*[right=$\mathcal N$-\textsc{app}-\rintro]
    {\oftp\Gamma{A}{B \to \UU} \\
    \oftp\Gamma{\mathcal N(i, A)}{\UU} \\
    \oftp\Gamma{b}{B} \\ \oftp\Gamma{a}{A}}
    {\oftp\Gamma{\nu(I_{\textsc{app}}(i,b), a)}{\mathcal N(I_{\textsc{app}}(i,b), A)}}
  \and
...
\begin{lstlisting}[mathescape=true]
(fprod $A_1:\UU$ Expr $\dots$ $A_n:\UU$ Expr)$ \;\longleftrightarrow\; \bigtimes_{j=1}^n A_j: \UU $
...
We similarly define
\begin{lstlisting}[mathescape=true]
(fpair $A:\UU$ Expr $B:A \to \UU$ Expr)$ \;\longleftrightarrow\; \tsm{x:A} B(x) : \UU $
...
Finally, we define
\begin{lstlisting}[mathescape=true]
(fcoprod $A_1:\UU$ Expr $\dots$ $A_n:\UU$ Expr)$ \;\longleftrightarrow\; \bigplus_{j=1}^n A_j: \UU $
