...
% TODO: continue here, converting functions into atomic principles
\begin{defn}
...
% TODO: use the induction principle, also remove the whole lemma about turning ind into a function
\begin{lstlisting}[mathescape=true]
...
Note that we can create recursive definitions here by, for example, encasing an \lstinline{fcoprod} inside a newtype, binding it to a name, and then using an \lstinline{inst} to instance that name inside its own definition.
...
% TODO: regularity requirements on the qualified names that fnewty can use, as well as making sure that only one newtype uses a given name in a whole project
...
\begin{defn}
  A \textit{value} is a realisation of an object which cannot be simplified, where bidirectional computation rules are not considered simplifications.
\end{defn}
...
We therefore will begin our description of value inference on a restricted subset of Feather expressions that may not include the \lstinline{inst} keyword.
Later, we expand the scope of inference to include knowledge gained only through use of \lstinline{inst}.
