...
fn partial_recursor_info(
    ...
    recursor_universe(env, meta_gen, ind, info).map(|recursor_universe| {
        ...
        if dependent_elimination(info) {
            type_former_ty = Expr::new_synthetic(ExprContents::Pi(Pi {
                parameter_name: major_premise.name.clone(),
                binder_annotation: major_premise.binder_annotation,
                parameter_ty: major_premise.metavariable.ty.clone(),
...
fn dependent_elimination(info: &PartialInductiveInformation) -> bool {
    !is_zero(&info.sort.0)
}
